				...
				<div class="card paint-card">
					<div class="card-body">
						...
						<form class="row g-3" action="addAppointment" method="post">
							...
							<div class="col-md-6">
								<label for="inputEmail4" class="form-label">Requirement</label> <input
									required type="text" class="form-control" name="requirement">
									<option value="">--select--</option>
							</div>

							<div class="col-md-6">
								<label for="inputPassword4" class="form-label">Worker</label> <select
									required class="form-control" name="work">
									<option value="">--select--</option>

									<%
									WorkerDao dao = new WorkerDao(DBConnect.getConn());
										List<Worker> list = dao.getAllWorker();
										for (Worker w : list){
										%>
										<option value="<%=w.getId()%>"><%=w.getFullName() %>(<%=w.getSpecialist() %>)
										</option>
										<%}
